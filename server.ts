...
// System prompt with full Zhang Zhiyang resume facts and MIMO model framing
const SYSTEM_PROMPT = `
...
1. 职业与专业定位：
   ...
   - 核心证书与技能：持有初级会计师证书，英语 CET-6 达到 577 分（具备无障碍的英语商务工作沟通能力），精通 SQL 数据库。
   - 软实力：曾任校、院辩论队队长，拥有极强的商务答辩、结构化表达与高压即兴沟通能力。

2. 头部实习经历背书：
   - 德勤中国（Deloitte China） · 数字化转型与出海咨询实习生（2026.06 - 2026.09）：
     - 业财融合诊断：主导采购、工程及资产等 3 大核心流程诊断，精准定位暂估滞后、费用跨期等 10+ 项业财断点，设计并落地数字化优化方案。
     - 数字化 ROI 测算：构建价值测算模型，对资金占用、维修成本等 6+ 场景进行测算，识别千万级成本优化空间。
     - AI 交付自动化：全栈开发并上线一款 AI 流程图自动生成工具，使咨询项目的流程建模效率大幅提升 80%+。
   - 上海市人民政府发展研究中心 · 财务合规实习生（2025.10 - 2026.03）：
     - 内控制度修订：修订 6+ 项政府采购与预算业务流程，梳理 80+ 项制度条款。
     - 费用报销审核：完成 300+ 份单据合规审核，反馈 20+ 异常，助力报销退回率下降 20%。
     - 机要流转：安全递送与追踪 20+ 份涉密和领导批示机要文件，分类归档 500+ 份财务凭证，100% 准确可追溯。
   - 上海电气集团上海电机厂有限公司 · 内控审计实习生（2025.01 - 2025.03）：
     - 控制测试：针对销售及采购的 8+ 个关键控制点进行测试，提出 3 项优化建议（其中 2 项已被整改采纳）。
     - 抽凭与底稿：独立抽查合同、收入凭证 200+ 份，完成 10+ 份采购与预付账款审计底稿编制。

3. 自研 AI+财经小产品（已开源并部署在 Vercel 稳定运行）：
   - AssetPilot-AI：集团固定资产全生命周期预测与资本开支（CapEx）决策平台。
   - SmartSpare-AI：大型设备制造的备件资产评估与维护决策平台（根据劣化曲线量化停机损失，计算采购 ROI 优先级）。
   - FundControl-AI：业财深度融合的集团资金流动性与穿透式风控系统，实时捕捉 [暂估滞后、费用跨期] 并多周期动态模拟。
...
1. 人设逻辑要一致：
   ...
   - 如果用户说“你好”、“下午好”等日常问候，不要直接背诵简历！应该以热情、贴心、略带活泼和幽默的语气和用户打招呼，向他们伸出欢迎之手，并可以顺带提及志洋极具反差的特质，例如：“您好呀！我是张志洋的专属 AI 贴身小秘书。很高兴和您聊天！我们志洋不仅在德勤做过数字化咨询，生活中还是个 182cm 拥有八块腹肌的无畏契约神话段位大神哦！今天有什么想了解的，都可以问我这只全能小秘书~”
...
// Safe Global logs for troubleshooting API failures in production
interface ErrorLog {
  timestamp: string;
  error: string;
  context: string;
}
let latestErrors: ErrorLog[] = [];

function logErrorToGlobal(error: any, context: string) {
  const errMsg = error?.message || String(error);
  latestErrors.unshift({
    timestamp: new Date().toISOString(),
    error: errMsg,
    context
  });
  if (latestErrors.length > 30) {
    latestErrors.pop();
  }
}

// Debug Environment Variables Endpoint (Safe)
app.get("/api/debug-env", (req, res) => {
  ...
  res.json({
    ...
    gemini_key: {
      ...
      prefix: geminiKey ? geminiKey.substring(0, 4) + "..." : "none"
    },
    custom_key: {
      exists: !!customKey,
      length: customKey ? customKey.length : 0,
      prefix: customKey ? customKey.substring(0, 4) + "..." : "none"
    },
    xiaomi_key: {
      exists: !!xiaomiKey,
      length: xiaomiKey ? xiaomiKey.length : 0,
      prefix: xiaomiKey ? xiaomiKey.substring(0, 4) + "..." : "none"
    },
...
app.post("/api/chat", async (req, res) => {
  ...
  async function callGeminiFallback() {
    const geminiKey = process.env.GEMINI_API_KEY;
    console.log("[callGeminiFallback] geminiKey exists:", !!geminiKey, "Length:", geminiKey ? geminiKey.length : 0);
    if (!geminiKey) {
      console.warn("[callGeminiFallback] GEMINI_API_KEY is not defined in process.env. Returning fallback.");
      logErrorToGlobal("GEMINI_API_KEY is not defined in process.env", "callGeminiFallback-missingKey");
    ...
    // Filter and map conversation history to Gemini 'user' and 'model' roles
    ...
    console.log("[callGeminiFallback] geminiContents constructed:", JSON.stringify(geminiContents, null, 2));

    if (geminiContents.length === 0) {
      console.warn("[callGeminiFallback] No user message found in history. Returning fallback.");
      logErrorToGlobal("No user message in history", "callGeminiFallback-emptyContents");
      return getHeuristicAnswer(lastUserMsg);
    }

    try {
      console.log("[callGeminiFallback] Sending generateContent request to gemini-3.6-flash...");
      ...
      console.log("[callGeminiFallback] generateContent response successfully received.");
      const replyText = response.text || "抱歉，未能正常生成回答。";
      return replyText.replace(/\*/g, "");
    } catch (err: any) {
      console.error("[callGeminiFallback] Error thrown by generateContent:", err.message, err.stack);
      logErrorToGlobal(err, "callGeminiFallback-generateContentError");
      throw err;
    }
  }
  ...
  if (activeKey && activeKey.trim() !== "") {
    try {
      ...
      if (replyText) {
        // Strip asterisks if any leaked through
      ...
      console.warn("Custom/Xiaomi API call failed. Seamlessly falling back to Gemini API:", apiError.message);
      logErrorToGlobal(apiError, "api-chat-custom-failure");
      ...
      try {
        ...
        logErrorToGlobal(geminiError, "api-chat-gemini-fallback-failure");
        return res.json({ 
    ...
    try {
      ...
      logErrorToGlobal(geminiError, "api-chat-gemini-direct-failure");
      return res.json({ 
        reply: "（系统提示：智能网关连接异常，已启动高保真本地知识库回答）\n\n" + getHeuristicAnswer(lastUserMsg) 
      });
    }
...
// Mounting static assets & Vite build pipeline
